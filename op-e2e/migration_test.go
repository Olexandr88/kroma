...
func TestMigration(t *testing.T) {
	...
	require.NoError(t, err)
	deployReceipt, err := geth.WaitForL2Transaction(tx.Hash(), l2Verif, 4*time.Duration(cfg.DeployConfig.L1BlockTime)*time.Second)
	require.NoError(t, err)
	// Set storage
	tx = types.MustSignNewTx(cfg.Secrets.Alice, types.LatestSignerForChainID(chainId), &types.DynamicFeeTx{
		ChainID:   chainId,
	...
	err = l2Seq.SendTransaction(ctx, tx)
	require.NoError(t, err)
	_, err = geth.WaitForL2Transaction(tx.Hash(), l2Verif, 4*time.Duration(cfg.DeployConfig.L1BlockTime)*time.Second)
	require.NoError(t, err)
	// Check storage value
	ctx, cancel = context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
